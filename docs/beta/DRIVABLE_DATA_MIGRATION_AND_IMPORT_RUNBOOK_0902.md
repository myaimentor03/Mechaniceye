...
| Schema sync | `shared/schema.ts` is the single canonical runtime schema; `server/db.ts`, `server/public-case-db.ts`, and `server/customer-auth.ts` all import it | In-tree, typechecked, `verify:migration-parity` green. |
...
ΓÇö it never connects, mutates, or hangs.

Also available (read-only):
- `DATABASE_URL=... node scripts/verify-seed-table-counts.mjs` ΓÇö seed row counts, all 8 tables + 270 total, exit 1 on mismatch.
...
- **Delivery outbox contract is enforced; no 0004 migration here.** No
  ...
  `delivery-outbox.contract.test.ts` against the in-memory repository plus the
  durable-capability gate `assertDurableDeliveryOutbox`. The commerce paid
  fulfillment path requires a durable outbox capability
  (`paid-fulfillment-eligibility.ts`) and fails internally when given an
  ephemeral outbox. Email/webhook dispatch in `server/routes.ts` is direct and
  **fail-closed**: it throws on a non-OK response (surfaced as a generic 502),
  throws when `MASTER_INTAKE_WEBHOOK_URL` is unset, and logs only via the
  PII-free observability helpers (`logEvent`/`logEventError`); no `console.*`
  calls remain in routes.
- **Schema drift fixed.** `shared/schema.ts` is the single canonical runtime
  schema; the stale duplicated copy (`server/shared/shared/schema.{ts,js}`) was
  removed in this integration. `server/db.ts`, `server/public-case-db.ts`, and
  `server/customer-auth.ts` all import the canonical root schema, and
...
2. Review of `migrations/0003_drivable_data_integrity_hardening.sql`
   ...
   **approves ┬º3.2** (optional hardening).
3. Review of the seed manifest
   ...
   `DRIVABLE_ALLOW_SEED_IMPORT=1` ΓÇö **approves ┬º4.**
...
- `npm run import:seed-data -- --apply` ΓÇö refuses to run while any manifest
  dataset is `importAllowedNow: false` unless `DRIVABLE_ALLOW_SEED_IMPORT=1`.
- `npm run nhtsa:batch -- --apply` / `npm run nhtsa:pack -- ... --apply` ΓÇö
  writes `drivable_vehicle_knowledge_packs`; requires the authenticated-target
  gate.
- `npm run create:seed-tables` ΓÇö raw `CREATE TABLE IF NOT EXISTS`; safe to
  re-run but only intended for fresh targets (see ┬º4).
- `node scripts/create-missing-tier2-nhtsa-batch.cjs` ΓÇö writes a CSV from a
...
- Any `psql "$DATABASE_URL" -f migrations/...` invoke is a schema-changing
  migration; run with `ON_ERROR_STOP=1` and a fresh backup per ┬º8.1.
