...
| Schema sync | `shared/schema.ts` is the single canonical runtime schema; `server/shared/shared/schema.{ts,js}` retain byte-identical copies used by `server/storage.ts` and `scripts/inspect-db-config.mjs` | In-tree, typechecked, `verify:migration-parity` green. |
...
- **Delivery outbox is provisioned, not wired.** `migrations/0004_drivable_delivery_outbox.sql`
  creates `drivable_delivery_outbox` (idempotent, optional) but no runtime code reads or
  writes it yet. The persist-first outbox contract lives in `server/jobs/delivery-outbox.ts`
  (enforced by `delivery-outbox.contract.test.ts`), and the commerce paid-fulfillment path
  requires a durable outbox capability (`paid-fulfillment-eligibility.ts`) that fails
  internally when given an ephemeral outbox. Email/webhook dispatch in `server/routes.ts` is
  direct and fail-closed: it throws on a non-OK response (surfaced as a generic 502), throws
  when `MASTER_INTAKE_WEBHOOK_URL` is unset, and logs only via the PII-free observability
  helpers (`logEvent`/`logEventError`); no `console.*` calls remain in routes.
- **Schema drift fixed.** `shared/schema.ts` is the single canonical runtime schema;
  `server/shared/shared/schema.{ts,js}` retain byte-identical copies required by
  `server/storage.ts`. `server/db.ts`, `server/public-case-db.ts`, and `server/customer-auth.ts`
  all import the canonical root schema, and `npm run verify:migration-parity` checks migrations
  against it.
