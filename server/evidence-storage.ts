...
export class RuntimeFileEvidenceStore implements EvidenceStore {
  ...
  private async mediaBytes(file: Express.Multer.File): Promise<Buffer> {
    if (file.buffer?.length) return file.buffer;
    if (file.path) return fs.readFile(file.path);
    throw new Error("Media has no readable content");
  }
    ...
    try {
      for (const file of files) {
        ...
        attachments.push({
          ...
          mimeType: file.mimetype, byteSize: buffer.length, status: "persisted",
...
export class S3PrivateEvidenceStore implements EvidenceStore {
  ...
  private async s3MediaBytes(file: Express.Multer.File): Promise<Buffer> {
    if (file.buffer?.length) return file.buffer;
    if (file.path) return fs.readFile(file.path);
    throw new Error("Media has no readable content");
  }
  ...
  async saveAudio(caseId: string, files: Express.Multer.File[]) {
    ...
    try {
      for (const file of files) {
        const buffer = await this.s3MediaBytes(file);
        if (!buffer?.length || file.size <= 0) throw new Error("Empty audio rejected");
        if (file.size > PHOTO_LIMITS.maxBytesEach) throw new Error("Audio is too large");
        const id = randomUUID();
        ...
        await this.client.send(new PutObjectCommand({
          ...
          CacheControl: "no-store",
          Metadata: evidenceObjectMetadata(safeCaseId, file.mimetype),
        }));
        writtenKeys.push(storageKey);
        attachments.push({
